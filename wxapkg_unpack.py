...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('usage: wxapkg_unpack.py filename [dirpath]')
        exit()
    ...
    with open(sys.argv[1], 'rb') as f:
        ...
        name = os.path.basename(f.name) + '_unpack'
        ...
        for i in range(fileCount):
            ...
            data.name = bytes.decode(f.read(data.nameLen))
